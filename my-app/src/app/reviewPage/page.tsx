    ...
    <div className="container mx-auto px-4 py-8">
      ...
      {previewMode ? (
        <div className="bg-white p-6 rounded-lg shadow-md">
          <h2 className="text-2xl font-bold mb-2">{form.getValues("reviewTitle")}</h2>
          <h3 className="text-xl mb-2">
            {form.getValues("bookTitle")} by {form.getValues("author")}
          </h3>
          <div className="flex items-center mb-2">
            {renderStars(form.getValues("rating"))}
            <span className="ml-2 text-gray-600">{form.getValues("rating")} stars</span>
          </div>
          <p className="text-sm text-gray-600 mb-4">
            Genre: {form.getValues("genre")} | Read on: {form.getValues("readDate")}
          </p>
          {form.getValues("containsSpoilers") && (
            <p className="text-red-500 mb-4">⚠️ This review contains spoilers</p>
          )}
          <p className="whitespace-pre-wrap">{form.getValues("reviewContent")}</p>
        </div>
      ) : (
        <Form {...form}>
